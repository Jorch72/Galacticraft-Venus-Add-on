        ...
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
            {
                for (int x = 0; x < GCVenusChunkProvider.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < GCVenusChunkProvider.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * GCVenusChunkProvider.CHUNK_SIZE_X + x, cz * GCVenusChunkProvider.CHUNK_SIZE_Z + z) / GCVenusChunkProvider.CRATER_PROB)
                        {
